...
Dygraph.Export.ComputeAttrs = function (dygraph, canvas) {
    ...
    var labels = dygraph.attr_("labels").slice(1).map(function (x) {
        return "\u2013 " + x;
    });
...
Dygraph.Export.asCanvas = function (dygraph, userOptions) {
    ...
    canvas.width  =  dygraph.width_  +  20;
...
Dygraph.Export.putLabel = function (ctx, divLabel, options, font, color) {
    ...
    top = top + options.magicNumbertop  +  10;
...
Dygraph.Export.drawLegend = function (canvas, dygraph, options) {
    ...
    for (var i = 0; i < legendArr.length; i++) {
        ...
        var rowWidth = curRow.reduce(function (prev, cur) {
...
Dygraph.Export.drawAnnotes = function (dygraph, ctx, options) {
    ...
    var fontsubs = ["style", "variant", "weight", "size", "family"].map(function (x) {
        return "font-" + x;
    });
    ...
    for (i = 0; i < annotedivs.length; i++) {
        ...
        var font = Object.keys(fontall).map(function (x) {
            return fontall[x];
        }).join(" ");
...
Dygraph.Export.VarsNames = function () {
    var names = new Array;
    for (var curname in Dygraph.Export.Vars) {
        names.push(curname);
    }
    return names;
};
...
Dygraph.Export.DownloadByID = function (id, usetitle) {
    ...
    if (!isArray(id)) {
        ...
        var datauri = "data:application/zip;base64," + zipobj.generate({
            type: "base64"
        });
